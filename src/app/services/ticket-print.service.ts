...
export class TicketPrintService {
  ...
  /**
   * El ticket se arma concatenando HTML y se inyecta con document.write en una
   * ventana same-origin, asi que cualquier valor dinamico sin escapar seria XSS
   * almacenado: la razon social y el domicilio fiscal los escribe el cliente en
   * el checkout, y quien imprime (y ejecutaria el script) es el personal.
   */
  ...
  private buildTicketHtml(invoice: InvoiceData, qrDataUrl: string = ''): string {
    ...
    const docName = esc(cust.name || 'Clientes Varios');
    // numeroALetras produce texto a partir de un number: no necesita escape.
    const totalLetras = numeroALetras(invoice.total);

    // Datos de empresa: los edita un admin, pero se escapan igual (defense in depth).
    const companyTitle = esc(c.commercialName || c.businessName || 'RESTAURANTE - CHIFA - POLLERÍA');
    const businessName = esc(c.businessName);
    const ownerName = esc(c.ownerName);
    const ruc = esc(c.ruc);
    const companyEmail = esc(c.email);
    const companyAddress = esc(
      c.address.street +
      (c.address.district ? ' - ' + c.address.district : '') +
      (c.address.province ? ' - ' + c.address.province : '') +
      (c.address.department ? ' - ' + c.address.department : '')
    );

    const serieNumber = esc(invoice.serieNumber);
    const issuedAt = esc(invoice.issueDate + ' ' + invoice.issueTime);
    const orderLabel = esc(invoice.orderNumber || invoice.orderId.slice(-6));

    const addressHtml = (!isBoleta && cust.address)
      ? '<div class="row"><span>Dirección:</span><span>' + esc(cust.address) + '</span></div>'
      : '';

    const qrHtml = qrDataUrl
      ? '<img src="' + esc(qrDataUrl) + '" alt="Código QR SUNAT" style="width:130px;height:130px;display:block;margin:0 auto;" />'
    ...
    const itemsRows = invoice.items.map(item => {
      ...
      const firstLine = esc(descLines[0] || '');
